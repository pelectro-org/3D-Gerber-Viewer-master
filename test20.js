...
function init(layers) {
    ...
    function updateArea(dw, dh) {
        ...
        $('.lengthgerbb').text(width);
        $('.widthgerbb').text(height);
    }
    updateArea(w, h);
...
$(function () {
    ...
    fileInput.on('change', function (ev) {
        ...
        if (isRar) {
            convertRarToZip(file, function (zipBlob) {
                handleZipFile(zipBlob, step2);
            });
        } else {
            handleZipFile(file, step2);
        }
    });

    function convertRarToZip(rarFile, callback) {
        var reader = new FileReader();
        reader.onload = function (e) {
            var rarContent = new Uint8Array(e.target.result);
            rarToZip(rarContent)
                .then(function (zipArray) {
                    var zipBlob = new Blob([zipArray], { type: 'application/zip' });
                    callback(zipBlob);
                })
                .catch(function (error) {
                    console.error('Error converting RAR to ZIP:', error);
                    alert('Failed to convert RAR to ZIP. Please try a ZIP file instead.');
                });
        };
        reader.readAsArrayBuffer(rarFile);
    }

    function handleZipFile(file, step2) {
        var zip = new JSZip();
        zip.loadAsync(file).then(function (zip) {
            processArchive(zip, step2, function (relativePath) {
                return zip.file(relativePath).async('string');
            });
        });
    }

    function processArchive(archive, step2, getFileContent) {
        var filePairs = [];
        ...
        var viewGerberButton = $('<button id="ViewButton">').text('View Gerber').css({
            ...
            var layerNum = filePairs.filter(function (pair) { return pair[1].val(); }).length;
            if (!layerNum)
                return alert('There has to be at least one selected layer!');

            var layers = [];
            filePairs.forEach(function (f) {
                var which = f[1].val();
                if (!which) return;

                getFileContent(f[0].name).then(function (content) {
                    var g;
                    try {
                        g = wG.load(content);
                    } catch (e) {
                        loadingOverlay.hide();
                        alert('Error while loading ' + f[0].name + ': ' + e);
                        console.log(e.stack);
                        return;
                    }
                    g.type = +which[1];
        ...
        step2.append('<br>', viewGerberButton);
        main.html('').append(step2);

        // Auto-click the View Gerber button after 1 second
        setTimeout(function() {
            viewGerberButton.click();
        }, 1000);
    }
});
